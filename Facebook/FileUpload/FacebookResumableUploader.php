...
class FacebookResumableUploader
{
    ...
    public function start($endpoint, FacebookFile $file)
    {
        ...
        return new FacebookTransferChunk($file, $response['upload_session_id'], $response['video_id'],
    ...
    {
        $request = new FacebookRequest($this->app, $this->accessToken, 'POST', $endpoint, $params, null,
            $this->graphVersion);
    ...
    public function transfer($endpoint, FacebookTransferChunk $chunk, $allowToThrow = false)
    {
        ...
        return new FacebookTransferChunk($chunk->getFile(), $chunk->getUploadSessionId(), $chunk->getVideoId(),
            $response['start_offset'], $response['end_offset']);
    }
